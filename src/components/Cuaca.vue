...
onMounted(async () => {
  inputOptions.selectInput = await fetchData(
    "https://training28.devbmkg.my.id/lokasi"
  );
...
async function ubahLokasi() {
  if (inputOptions["provinsi"] == null) {
    ...
    inputOptions["selectInput"] = await fetchData(
      "https://training28.devbmkg.my.id/lokasi?prov=" + inputOptions["provinsi"]
    );
    lokasi.value = "Pilih Kab";
  } else if (inputOptions["kabupaten"] == null) {
    inputOptions["kabupaten"] = lokasi.value;
    inputOptions["selectInput"] = await fetchData(
      "https://training28.devbmkg.my.id/lokasi?kabkota=" +
        inputOptions["kabupaten"]
    );
...
async function submitLokasi() {
  ...
  cuaca.data = await fetchData(
    "https://training28.devbmkg.my.id/cuaca-kec/" + inputOptions.kecamatan[0]
  );
  cuaca.data = cuaca.data["result"][0];
  cuacaStore.data = await fetchData(
    "https://training28.devbmkg.my.id/cuaca-kec-detail/" +
      inputOptions.kecamatan[0]
  ); // Fetch data cuaca lengkap untuk halaman CuacaView.vue
...
<template lang="">
  <div class="container">
    ...
    >
      ...
      <div>{{ cuaca.data["hari"] }} {{ cuaca.data["jam"] }}</div>
      <img
      ...
      <div>
        {{ cuaca.data["t"] }}<span>&#176;</span>C {{ cuaca.data["hu"] }}%
      </div>
    </div>
    <!-- Tampilan Cuaca setelah Lokasi Berhasil Dipilih -->
  </div>
